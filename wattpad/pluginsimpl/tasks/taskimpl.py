...
class TaskImpl:
    ...
    async def get_new_chapters(self, plugin: lightbulb.Plugin) -> None:
        try:
            ...
            filteredStories = {
                guild: story for guild, story in stories.items() if story
            }
                ...
                try:
                    ...
                    for storyRec in nonEmptyStories:
                        ...
                        if channel:
                            ...
                            hasSendPerms = await self.__has_send_perms(
                                plugin, guild, channel
                            )

                            if hasSendPerms:
                                # check for the chapter update for story
                                update = await scraper.get_new_chapter(
                                    storyRec["url"], storyRec["lastupdated"]
                                )
                                ...
                                if update.IsSuccess:
                                    ...
                                    if not customMsg:
                                        categoryCustomMsgs = (
                                            await dataUtil.get_messages()
                                        )

                                        if (
                                            guild in categoryCustomMsgs
                                            and "story" in categoryCustomMsgs[guild]
                                        ):
                                    ...
                                    # get story title
                                    title = await wattpadUtil.get_story_title_from_url(
                                        storyRec["url"]
                                    )
                                    ...
                                    for eachStory in stories[guild]:
                                        if eachStory["url"] == storyRec["url"]:
                                            eachStory[
                                                "lastupdated"
                                            ] = update.UpdatedDate.strftime(
                                                "%Y-%m-%d %H:%M:%S"
                                            )

                                            updateResult = (
                                                await dataUtil.update_stories(stories)
                                            )

                                            if not updateResult:
                                                self.logger.error(
                                                    "%s.get_new_chapters Error while updating stories for server: %s, story: %s",
                                                    self.filePrefix,
                                                    guild,
                                                    storyRec["url"],
                                                )
                                ...
                                self.logger.error(
                                    "Error %s.get_new_chapters method: no send permissions for channel: %s, server: %s, story: %s",
                                    self.filePrefix,
                                    channel,
                                    guild,
                                    storyRec["url"],
                            ...
                            self.logger.error(
                                "Error %s.get_new_chapters: no channel for updates is setup for server: %s, story: %s",
                                self.filePrefix,
                                guild,
                                storyRec["url"],
                            )
                    ...
                    self.logger.error(
                        "Error occured in %s.get_new_chapters method",
                        self.filePrefix,
                        exc_info=1,
                    )
                    pass
            ...
            self.logger.fatal(
                "Exception occured in %s.get_new_chapters method",
                self.filePrefix,
                exc_info=1,
            )
            return False
    ...
    async def get_new_announcements(self, plugin: lightbulb.Plugin) -> None:
        try:
            ...
            filteredAuthors = {
                guild: author for guild, author in authors.items() if author
            }
            ...
            for guild, authorList in filteredAuthors.items():
                try:
                    ...
                    for authorRec in nonEmptyAuthors:
                        ...
                        if channel:
                            ...
                            if hasSendPerms:
                                ...
                                update = await scraper.get_new_announcement(
                                    authorRec["url"], authorRec["lastupdated"]
                                )
                                ...
                                if update.IsSuccess:
                                    ...
                                    if not customMsg:
                                        ...
                                        if (
                                            guild in categoryCustomMsgs
                                            and "announcement"
                                            in categoryCustomMsgs[guild]
                                        ):
                                            msg = categoryCustomMsgs[guild][
                                                "announcement"
                                            ]
                                    ...
                                    # get story title
                                    title = await wattpadUtil.get_author_name(
                                        authorRec["url"]
                                    )
                                    ...
                                    em = hikari.Embed(
                                        title="Announcement:",
                                        description=f"{update.NewUpdate}",
                                        color=0xFF500A,
                                    )

                                    await plugin.bot.rest.create_message(
                                        channel, response, embed=em
                                    )
                                    ...
                                    for eachAuthor in authors[guild]:
                                        if eachAuthor["url"] == authorRec["url"]:
                                            ...
                                            updateResult = (
                                                await dataUtil.update_authors(authors)
                                            )

                                            if not updateResult:
                                                self.logger.error(
                                                    "%s.get_new_chapters Error while updating authors for server: %s, author: %s",
                                                    self.filePrefix,
                                                    guild,
                                                    authorRec["url"],
                                                )
                                ...
                                self.logger.error(
                                    "Error in %s.get_new_announcements method: no send permissions for channel: %s, server: %s, author: %s",
                                    self.filePrefix,
                                    channel,
                                    guild,
                                    authorRec["url"],
                            ...
                            self.logger.error(
                                "Error in %s.get_new_announcements method: no channel for updates is setup for server: %s, author: %s",
                                self.filePrefix,
                                guild,
                                authorRec["url"],
                            )
                    ...
                    self.logger.error(
                        "Error occured in %s.get_new_announcements method",
                        self.filePrefix,
                        exc_info=1,
                    )
                    pass
            ...
            self.logger.fatal(
                "Exception occured in %s.get_new_announcements invoked",
                self.filePrefix,
                exc_info=1,
            )
            return False

    async def cleanup_stored_data(self, plugin: lightbulb.Plugin) -> None:
        """Remove servers in which the bot's not present."""

        try:
            self.logger.info("%s.cleanup_stored_data method invoked", self.filePrefix)
            ...
            guilds = await plugin.bot.rest.fetch_my_guilds()

            new_stories = {
                k: v
                for k, v in stories.items()
                if k in [str(guild.id) for guild in guilds]
            }
            ...
            author_update = await dataUtil.update_authors(new_authors)
            story_update = await dataUtil.update_stories(new_stories)
            msg_update = await dataUtil.update_messages(new_messages)
            channel_update = await dataUtil.update_channels(new_channels)

            if not author_update:
                self.logger.error(
                    "%s.cleanup_stored_data Error while updating authors",
                    self.filePrefix,
                )

            if not story_update:
                self.logger.error(
                    "%s.cleanup_stored_data Error while updating stories",
                    self.filePrefix,
                )

            if not msg_update:
                self.logger.error(
                    "%s.cleanup_stored_data Error while updating messages",
                    self.filePrefix,
                )

            if not channel_update:
                self.logger.error(
                    "%s.cleanup_stored_data Error while updating channels",
                    self.filePrefix,
                )
    ...
    async def __has_send_perms(
        ...
        try:
            self.logger.info(
                "%s.__has_send_perms method invoked for channel: %s",
                self.filePrefix,
                channel,
            )
            ...
            self.logger.fatal(
                "Exception occured in %s.__has_send_perms method for channel: %s",
                self.filePrefix,
                channel,
                exc_info=1,
            )
