...
from wattpad.models.result import ResultNewUpdate
import aiohttp
...
class Scraper:
    def __init__(self) -> None:
        self.filePrefix= "wattpad.scraper.scraper"
    ...
    async def get_new_chapter(self, url:str, lastchecked:datetime) -> ResultNewUpdate:
        try:
            self.logger.info("%s.get_new_chapter method invoked for story: %s, lastchecked: %s", self.filePrefix, url, lastchecked)
            ...
            try:
                ...
                self.logger.fatal("Exception occured in %s.get_new_chapter method while sneding request to stpory: %s, last checked: %s", self.filePrefix, url, lastchecked, exc_info=1)
            ...
            if response.status == 200:
                ...
                is_newly_updated= await self.__is_newly_updated(soup)
                ...
                #     self.logger.fatal("Exception occured in %s.get_new_chapter method while chekcing for lastupdated", self.filePrefix, exc_info=1)
                #     pass
                        ...
                        for item in all_chapters:
                            ...
                            for a in reversed(each_chapter):
                                ...
                                try:
                                    ...
                                    self.logger.error("Exception occured in %s.get_new_chapter while sending a request to the URL %s", self.filePrefix, url,exc_info=1)
                                    continue

                                if resp.status==200:
                                    update= await self.__find_new_chapter(chapterReq, lastchecked, chapterLink, url, part)
                                    ...
                                    # else:
                                    #     self.logger.error("Error occured in %s.get_new_chapter. Error: script data is not found for a chapter: %s", self.filePrefix, chapterLink)
                                    ...
                                    #         self.logger.info("%s.get_new_chapter method ended for story: %s", self.filePrefix, url)
            ...
            self.logger.fatal("Exception occured in %s.get_new_chapter method for story: %s, lastchecked: %s", self.filePrefix, url, lastchecked, exc_info=1)
            pass

    async def get_new_announcement(self, url:str, lastchecked: datetime) -> ResultNewUpdate:
        try:
            self.logger.info("%s.get_new_announcement method invoked for url: %s, last checked: %s", self.filePrefix, url, lastchecked)
            ...
            try:
                ...
                self.logger.error("Exception occured in %s.get_new_announcement method while sending request to author url: %s", self.filePrefix, conversations_url, exc_info=1)
                return None
                ...
                if announcements:
                    ...
                    if from_name.text.lower()==author_name.lower():
                        ...
                        if lastUpdateTime > lastcheckeddate:
                            ...
                            self.logger.info("%s.get_new_announcement method ended for author: %s", self.filePrefix, url)
            ...
            self.logger.fatal("Exception occured in %s.get_new_announcement method invoked for url: %s, last checked: %s", self.filePrefix, url, lastchecked,exc_info=1)
            raise e

    async def __find_new_chapter(self, chapterReq, lastchecked, chapterLink:str, url:str, part) -> ResultNewUpdate:
        try:
            self.logger.info("%s.__find_new_chapter method invoked")
            ...
            self.logger.fatal("Exception occured in %s.__find_new_chapter method",exc_info=1)
            raise e
    ...
    async def __script_with_data(self, scripts):
        try:
            self.logger.info("%s.script_with_data method invoked", self.filePrefix)
            ...
            self.logger.fatal("Exception occured in %s.script_with_data method while getting script data", self.filePrefix, exc_info=1)
            pass
    ...
    async def __get_usable_json(self, rawJSON) -> dict:
        ...
        """
        try:
            self.logger.info("%s.get_usable_json method invoked", self.filePrefix)
            while True:
                try:
            ...
            self.logger.fatal("Exception occured in %s.get_usable_json method", self.filePrefix, exc_info=1)
            pass

    async def __is_newly_updated(self, soup) -> bool:
        try:
            self.logger.info("%s.__is_newly_updated method invoked")
            ...
            try:
                ...
                self.logger.fatal("Exception occured in %s.__is_newly_updated method while chekcing for lastupdated", self.filePrefix, exc_info=1)
                pass
            ...
            self.logger.fatal("Exception occured in %s.__is_newly_updated method",exc_info=1)
            raise e
