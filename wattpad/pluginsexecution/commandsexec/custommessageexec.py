...
class CustomMessageExec:
    ...
    async def set_custom_message_for_story(self, guildid: str, storyurl:str, message:str) -> Result:
        try:
            ...
            if not story_urls:
                return Result(False, "No story found with the title", IsInvalidTitle=True)

            else:
                if len(story_urls) > 1:
                    return Result(False, "Multiple stories found with this title", HasMultipleResults=True)
            ...
            return Result(False, "Unknown error", UnknownError=True)
